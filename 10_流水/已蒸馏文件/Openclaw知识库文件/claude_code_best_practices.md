...
## 今日主题：Zero-Shot 复杂应用生成 —— 别把 PRD 当许愿池，要把它压成可验收的垂直切片流水线

### 为什么现在该学这个
路线图里下一个未掌握主题，就是终极专家篇第 1 项：
**给 Claude Code 一份 PRD + UI 截图，让它从零生成复杂应用。**

这件事最容易被吹成魔法，实际却最容易翻车。
真正的关键不是“Claude 能不能一把生成”，而是：
**你能不能把需求、视觉、边界、验证和回滚，压成一个连续可控的工作流。**

一句话：
**Zero-Shot 不是一把梭，而是 spec-first 的多阶段闭环。**

---

## 官方与高质量实践指向的共同结论

### 1. 先让 Claude 看图、看需求、看边界，再让它写
Anthropic 官方 `Common workflows`、前面已经验证过的 `Plan Mode` 思路，以及官方工程化插件 `feature-dev` 的七阶段流，都在说同一件事：
**复杂任务先探索和澄清，不要上来就改文件。**

对“从零生成应用”尤其如此。
因为 PRD 和截图天然包含大量隐含假设：
- 哪些是必须实现的核心路径
- 哪些只是视觉参考
- 哪些是首版先不做的非目标
- 数据模型、鉴权、部署、测试各自最小边界在哪

如果不先让 Claude 把这些东西显式化，它就会开始脑补，最后产出一堆“像产品、不像项目”的半成品。

所以第一原则很硬：
**先把 PRD 和截图翻译成规格，再翻译成代码。**

---

### 2. 截图不是装饰，它是验收合同的一部分
前面关于 UI 闭环的经验已经证明：
**截图最值钱的用途不是“让 Claude 灵感更足”，而是提供可对比的视觉验收标准。**

应用从零生成时，截图至少承担三种角色：
1. **结构合同**：页面区块、布局层级、组件关系。
2. **交互合同**：哪些状态必须出现，主流程如何流转。
3. **视觉边界**：间距、密度、信息重心、按钮优先级。
...
- 哪些视觉细节要 1:1 追
- 哪些只取精神，不做像素级复刻
- 哪些状态图目前缺失，需要它先假设再标注风险

这样截图才是合同，不是氛围组。

---

### 3. 真正稳的起手式不是“生成整个 app”，而是“先打穿一条垂直切片”
Anthropic 官方工作流一直在强调：复杂工作要拆成阶段、拆成可验证单元。
结合 `feature-dev` 这类流程化实践，我的判断很明确：
**Zero-Shot 最忌讳一次性把前端、后端、数据库、鉴权、部署全抛给 Claude。**
...
比如做一个后台系统，第一刀只打通：
**登录 → 进入首页 → 看到列表 → 新建一条记录 → 能保存并刷新显示。**

只有这条链路跑通了，Claude 才算不是在空中搭房子。

一句话：
**先做最小可玩的产品骨架，不做第一天就想毕业的全量系统。**

---

### 4. 脚手架要先薄，目录和验证门要先硬
Zero-Shot 场景里，Claude 很容易“热情过剩”：
- 一上来创建几十个文件
- 提前铺太多抽象层
- 顺手把未来半年才要的能力也搭进去

这玩意看起来勤快，其实是污染上下文、放大维护成本。

更稳的方式是：
1. **先定技术栈与脚手架命令**
2. **先定目录骨架与模块职责**
3. **先定测试 / lint / type-check / build 的验证门**
4. **再允许它往里填实现**

也就是说，第一批产物最重要的不是功能多，而是：
- 跑得起来
- 测得动
- 改得清楚
- 回滚得掉

如果连验证门都没立住，后面所有“自动生成”都只是把技术债生产得更快。

---

### 5. Zero-Shot 也必须强制“计划层”和“执行层”分离
前面学过的 `opusplan`、Plan Mode、subagent 路由，到这里不是可选项，是刚需。
...
这样做的好处非常直接：
**最贵的思考留给规划，最重的体力活交给执行。**
别让同一个会话一边想架构、一边冲动建一堆文件，最后脑子和目录一起发臭。

---

## 小龙儿给主人的落地工作流：PRD + 截图 → 首版应用的六段式流水线

### 第 0 步：先写“非目标清单”
在把 PRD 给 Claude 前，先补 4 类边界：
- **本轮必须做**
- **本轮明确不做**
- **允许假设的地方**
- **必须提问确认的地方**

这是 Zero-Shot 最容易被忽视、但最值钱的一步。
因为你不写非目标，Claude 就会把“以后可能需要”全都当成“现在就要做”。

---

### 第 1 步：进入 Plan Mode，把 PRD 压成交付规格
Prompt 模板：

“进入 Plan Mode。阅读这份 PRD 和这些界面截图。先不要生成代码。请输出：
1. MVP 的 1 条主用户路径；
2. 需要的页面与状态；
3. 数据模型与接口草案；
4. 哪些需求不明确；
...
- 初始化项目
- 建最小目录
- 接好 lint / type-check / test / build
- 写 README 或 `CLAUDE.md` 说明如何运行
- 搭一个空页面/空接口证明骨架能跑

记住：
**第一轮不是做业务，是建战场。**

如果骨架阶段就开始铺复杂权限、缓存层、插件系统、万能组件库，那八成已经偏了。
...
这一步是 Zero-Shot 里最重要的“去幻觉”动作。
因为只要主链路没通，其他漂亮页面都是假把式。

---

### 第 4 步：用截图做视觉回归，不靠嘴验收
一旦主链路页面出来，就要求 Claude：
- 打开页面
- 截图
- 对照参考图
- 列出结构差异、间距差异、状态差异
...
Zero-Shot 真正能落地，靠的不是第一天生成 80%，而是后面每次都能稳定长出来。

---

### 第 6 步：Checkpoint + Git 双保险
从零生成应用时，最怕 Claude 一轮热情改坏半个仓库。
所以必须双保险：
1. **Checkpoint**：每个阶段前后打点，方便秒回退。
2. **Git 小提交**：每条切片单独提交，保证 diff 清晰可审。

别等系统长成一大坨再回看历史，那时候你只会想骂人。

---

## 主人最容易犯的 5 个蠢错

### 蠢错 1：把 PRD 直接当 prompt 扔过去
没有阶段、没有边界、没有验收门，Claude 只会把你的野心原样放大成混乱。

### 蠢错 2：截图只给一张首页美图
没有关键状态、空态、错误态、编辑态，Claude 只能靠猜补完。

### 蠢错 3：第一轮就要求“全栈全功能全上线”
这不是 ambitious，这是弱智。先打穿一条链路，再扩张。

### 蠢错 4：只看它写得快，不看验证门
没跑测试、没 build、没截图回归的“已完成”，全都不算完成。

### 蠢错 5：不写非目标
你不写“这版不做权限系统/不做支付/不做后台配置”，Claude 就会手痒乱长。

---

## 给《光与朽》或我们后续工具链的直接用法
如果后面我们要让 Claude Code 从零起一套：
- 数值配置后台
- 广告素材管理小工具
- 测试数据看板
- AI 视频线索整理台
...
都直接套这条：
**PRD + 截图 → Plan Mode 压规格 → 最薄脚手架 → 单条垂直切片 → 截图回归 + 自动验证 → Checkpoint/Git 扩张。**

这套对一人开发尤其狠，因为它把“我脑子里大概有产品”强行变成“Claude 能被我稳定驱动的交付机器”。
...
**Claude Code 的 Zero-Shot 复杂应用生成，核心不是一把生成，而是先把 PRD 和截图压成可验证的垂直切片流水线。**

先定非目标，后压规格；先搭薄骨架，后打一条主链；每步都过测试、构建和截图回归。这样 Claude 才是在盖楼，不是在堆垃圾。

---

## 参考来源
- Anthropic 官方文档：Common workflows（Plan Mode / staged implementation / verification）
- Anthropic 官方文档：Best practices for Claude Code（self-verification / provide tests and screenshots / clear success criteria）
- Anthropic 官方文档：Vision（多图输入、截图理解、图像尺寸与成本边界）
- Anthropic 官方插件：feature-dev（需求澄清 → 设计方案 → 实现 → review → 总结 的阶段化流程）
- Builder.io：How I use Claude Code (+ my best tips)（复杂任务拆阶段、先计划后执行的实战经验）

---

## 今日主题：Subagents + Git Worktree 隔离并行流 —— 让 Claude Code 真正像主脑带队，而不是主线程亲自搬砖

### 为什么今天该学这个
按路线图顺序，Zero-Shot 复杂应用生成之后，下一个未掌握的大项是：
**多 Agent 协同生态构建。**
但这一块如果讲太大，就会变空话。所以今天只学一刀最值钱、最能落地的具体工作流：
**用 subagents + git worktree + hooks + checkpoints，搭一套可并行、可回滚、可验收的 Claude Code 协作流。**

一句话：
**主线程负责拆题和验收，worker 负责干净地各自施工。**

---

## 官方与高质量实践给出的硬结论

### 1. Subagent 的第一价值不是“更聪明”，而是把脏上下文隔离出去
Anthropic 官方关于 Claude Code 自主化的说明里，已经把方向讲得很清楚：
- **subagents 用来委派专项任务**，比如主 agent 搭前端时，让子 agent 独立起后端 API；
- 价值在于**并行开发**与**上下文解耦**，不是为了热闹多开几个 Claude。

这点和我们前面学过的 Plan Mode 完全接得上：
主线程不该被日志、grep 结果、局部文件细节塞爆。那些噪音应该被 worker 吃掉，只把摘要和结果回传。

### 2. 真并行要配 worktree，不然多 agent 很容易互踩
Anthropic Advanced Patterns 公开资料里最值钱的一句，不是“可以并行”，而是：
**每个 Claude 实例放进不同的 git worktree，才能避免文件冲突。**

这意味着：
- 只读摸底类 worker 可以共享主工作区；
- **任何会改代码的并行 worker，优先给独立 worktree。**

否则两个 agent 同时改一套文件，主线程最后收的不是成果，是事故现场。

### 3. 好的 subagent 描述要像触发器，不要像自我介绍
Builder 的 subagent 实战文章给出的经验非常硬：
- **description 要尖锐**；
- **工具要收窄**；
- **权限面要尽量小**；
- 能并行跑的 worker 再开 background。

说白了，subagent 配置不是写人设，而是写派工规则。
烂写法：
- “帮助写代码的 agent”

好写法：
- “只负责扫描陌生仓库入口、依赖链和关键数据流；不改文件；返回结构摘要。”

Claude 真正能不能自动派对工，description 比空洞 prompt 重要得多。

### 4. Hooks 的作用不是炫技，是把收工门槛写死
Anthropic 官方自主化说明里明确提到：
**hooks 会在关键节点自动触发动作。**
这玩意最值钱的不是“省一步命令”，而是把流程从“最好这样做”变成“必须这样做”。

对多 agent 流最该先上的 hook 只有三类：
1. **改文件后自动跑相关检查**（test/lint/type-check）
2. **Stop 前检查是否已验证**
3. **危险命令前拦截**

别一口气配二十个 hook，先把这三道闸装上，收益已经很大。

### 5. Checkpoint 是多 agent 流的保险丝
Anthropic 官方把 checkpoints 和 subagents、hooks 放在同一波自治增强里，不是偶然。
因为并行 worker 的最大风险不是“不会干活”，而是：
**会很勤快地把错改大。**

所以正确姿势是：
- 开工前打 checkpoint
- 每完成一个批次再打 checkpoint
- 主线程验收不通过，先回退，再重新派工

Checkpoint 管秒撤销，Git 管清晰历史；两个都要。

---

## 最能落地的工作流：主脑 + 三类工人

### 第 1 类：Explorer（只读侦察兵）
职责：
- 摸目录
- 找入口
- 追调用链
...
推荐配置：
- 模型：Sonnet
- 工具：Edit / Read / Bash（按需）
- **并行时使用独立 worktree**
- 每个 worker 只负责一个垂直子目标

典型例子：
- worker A：登录接口与鉴权链路
- worker B：列表页 UI 与状态管理
- worker C：测试补齐与回归检查

### 第 3 类：Reviewer / Verifier（验收工人）
职责：
- 只读 review diff
- 检查风险点
- 回传阻塞项
...
4. **所有会写代码的并行 worker 一律独立 worktree**
   - 避免互踩
   - 方便局部回滚
...
### 蠢错 1：多个 agent 同时改同一工作区
这不是协作，是互相踩踏。
并行改代码就上 worktree。

### 蠢错 2：description 写成空话
description 不清，Claude 就不会稳定委派。
它必须像“触发条件 + 职责边界”。

### 蠢错 3：把一个 worker 配成万能工
又查、又改、又测、又总结，最后最容易失控。
单一职责才稳。

### 蠢错 4：不给 hook，只靠 agent 自觉验收
这等于相信实习生会自己加班补作业。
流程闸门必须机械化。
...
## 给《光与朽》或我们工具链的直接用法
如果后面要把 Claude Code 更深接进《光与朽》或周边工具开发，我建议第一批就这样拆：

1. **unity-explorer**：只读扫 `Assets/Scripts` 结构与引用链
2. **combat-implementer**：只负责某个战斗子系统，独立 worktree 施工
3. **config-reviewer**：只读检查 ScriptableObject / JSON 配表风险
4. **release-checker**：只跑构建、lint、资源检查和上线前清单
...
这才是把 Claude Code 从“会写代码”升级成“能带队开发”。
...
**多 Agent 协作真正的关键，不是同时开更多 Claude，而是让每个 worker 有独立边界、独立上下文、独立回滚面。**

subagents 负责分工，worktree 负责隔离，hooks 负责闸门，checkpoints 负责后悔药。四件事一起上，Claude Code 才像主脑带队，不像一群高配实习生互相踩文件。

---

## 参考来源
- Anthropic 官方资讯：Enabling Claude Code to work more autonomously（subagents / hooks / checkpoints / parallel workflows）
- Anthropic 官方 Webinar：Claude Code Advanced Patterns: Subagents, MCP, and Scaling to Real Codebases（parallel instances / git worktrees / orchestration）
- Builder.io：Claude Code Subagents: How to Create, Use, and Debug Them（sharp description / narrow tools / smallest permission surface / background workers）
